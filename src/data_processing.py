...
def _extract_datetime_features(df: pd.DataFrame, datetime_col: str = "TransactionStartTime") -> pd.DataFrame:
...
def _aggregate_customer(df: pd.DataFrame, customer_id_col: str = "CustomerId", amount_col: str = "Amount") -> pd.DataFrame:
...
def build_feature_dataset(raw_df: pd.DataFrame) -> pd.DataFrame:
	...
	# Standardize numeric
	num_cols = [c for c in cust_df.columns if c != "CustomerId" and pd.api.types.is_numeric_dtype(cust_df[c])]
	if num_cols:
...
def compute_information_value(df: pd.DataFrame, target_col: str, bins: int = 10) -> pd.DataFrame:
	...
	def _iv_for_feature(x: pd.Series) -> float:
		...
		grouped = tmp.groupby("bin").agg(good=("target", lambda s: (s == 0).sum()), bad=("target", lambda s: (s == 1).sum()))
